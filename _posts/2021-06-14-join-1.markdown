...
Except for the above standard join types, there are some related join terms that you will see in many scenarios.
That is part of the reason that makes understanding joins confusing.

**```NATURAL JOIN```** is a join type similar to above standard operations included in _ANSI 92_, but not supported by all the databases. PostgreSQL, MySQL and Oracle support natural joins; Microsoft T-SQL and IBM DB2 do not.
...
The **USING clause** is used to explicitly specify the matching column.
Any columns mentioned in the USING list will appear only once,
NATURAL JOIN and USING Clause are mutually exclusive and supported together[[](https://troels.arvin.dk/db/rdbms/)].

```NATURAL JOIN``` creates an implicit join clause based on the common columns (defined as having the same name) in the two tables being joined. 
A NATURAL JOIN can be an INNER join, a LEFT OUTER join, or a RIGHT OUTER join, for example, you can write ```SELECT *
FROM employee NATURAL LEFT JOIN department;```.
The default is INNER join.

The NATURAL JOIN checks the definition of both tables (in the data catalog or metadata) and looks for like named columns. 
It should not have a qualifier(table name or Alias) in the referenced columns.
While the join conditions can include one or more columns, you have no choice in selecting the join columns.
The major concern of the natural join is that it assumes stability and consistency in column names which can change during vendor mandated version upgrades, while in many database environments the column names are controlled by an outside vendor, not the query developer. The core danger it omitting the column validation step. 
...
When the database is changed without notifying the query developer, the results might not be what the query developer is expecting for.
This [blog](https://blog.mclaughlinsoftware.com/2008/05/24/unnatural-outcome-of-natural-joins/) shows a concerete example.


**EQUI-JOIN**, **NON-EQUI JOIN** and **Theta JOIN** are a group of comparator-based join.
They are just the name for the joins with or without the equal condition.
EQUI-JOIN refers to the joins that uses only equality comparisons in the join-predicate, i.e. using other comparison operators (such as <) disqualifies a join as an equi-join. The examples we saw so far are all EQUI-JOINs.
All the NATURAL JOINs are EQUI-JOINs.
Similarly, NON-EQUI JOIN refers to the joins that use comparison operators like <, >, <=, >=, !=, and <> and the BETWEEN operator (but not =).
For example, you can write ``` SELECT * FROM employee JOIN department ON employee.DepartmentID != department.DepartmentID;
```.
A theta join allows for arbitrary comparison relationships. 
An equijoin is a theta join using the equality operator.
Both NON-EQUI JOIN and EQUI-JOIN are theta joins.
Natural join is a subset of Equi join, which is a subset of Theta join.
